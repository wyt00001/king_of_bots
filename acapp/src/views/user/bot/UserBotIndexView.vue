              ...
              <td>
                ...
                <div class="game-modal" :id="'update-bot-modal-' + bot.id" tabindex="-1" v-if="bot.show_update_modal">
                  ...
                  <div>
                    <div>
                      <label for="add-bot-title">名称</label>
                      <input style="width: 85%" v-model="editingBot.title" type="text" placeholder="请输入Bot名称">
                    </div>
                    <div>
                      <label for="add-bot-description">简介</label>
                      <textarea style="width: 85%; margin-top: 10px" v-model="editingBot.description" rows="3"
                        placeholder="请输入Bot简介"></textarea>
                    </div>
                    <div>
                      <label for="add-bot-code" class="form-label">代码</label>
                      <VAceEditor v-model:value="editingBot.content" @init="editorInit" lang="c_cpp" theme="textmate"
                        style="height: 300px" :options="{
                  ...
                  <div>
                    <div class="error-message">{{ botadd.error_message }}</div>
                    <button type="button" @click="update_bot">保存修改</button>
...
export default {
  ...
  setup() {
    ...
    const update_bot = () => {
      const bot = bots.value.find(b => b.id === editingBot.id);
      if (!bot) return;
      ...
      $.ajax({
        ...
        success(resp) {
          if (resp.error_message === "success") {
            // 写回原数据
            bot.title = editingBot.title;
            bot.description = editingBot.description;
            bot.content = editingBot.content;

            // 关闭 modal
            show_update_modal_handler(editingBot.id, false);
            refresh_bots();
          } else {
            botadd.error_message = resp.error_message;
          }
      ...
      for (const bot of bots.value) {
        if (bot.id === bot_id) {
          ...
          if (is_show) {
            ...
            editingBot.title = bot.title;
            editingBot.description = bot.description;
            editingBot.content = bot.content;
          }
        }
        new_bots.push(bot);
      }
